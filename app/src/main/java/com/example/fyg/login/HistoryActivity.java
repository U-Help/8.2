...
public class HistoryActivity extends Activity {
    ...
    private ArrayList<String> getData() {
        ...
        for(i=0;i<history.length;i++){
            list.add("项目 "+Integer.toString(i+1)+"\n"+
                    ...
                    "取货地点：" +history._histories[i].srcplace+"\n"+
                    "收货地点："+history._histories[i].dstplace+"\n"+
                    "快件大小："+history._histories[i].size+"\n"+
                    "交易价格："+history._histories[i].price+"\n"+
                    "交易完成时间："+history._histories[i].accept_time);
        }
        for(;i<history.length+history1.length;i++){
            list.add("项目 "+Integer.toString(i+1)+"\n"+
                    ...
                    "取货地点：" +history1.histories[i-history.length].srcplace+"\n"+
                    "收货地点："+history1.histories[i-history.length].dstplace+"\n"+
                    "快件大小："+history1.histories[i-history.length].size+"\n"+
                    "交易价格："+history1.histories[i-history.length].price+"\n"+
                    "交易完成时间："+history1.histories[i-history.length].accept_time);
        }
        mListView.setAdapter(adapter);
        return list;
    }
    ...
    public void getInfo(){
        ...
        Request request = new Request.Builder()
                .url("http://47.100.116.160:5000/item/pro_history")//acc_history
                .post(body)
                .build();
    ...
    public void getinfo(){
        ...
        RequestBody body = RequestBody.create(JSON, jsonStr);
        Request request = new Request.Builder()
                .url("http://47.100.116.160:5000/item/acc_history")//acc_history
                .post(body)
                .build();
